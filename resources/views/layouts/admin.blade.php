...
<head>
    ...
    <title>{{ config('app.clinic_name', 'Antrix') }} Admin — @yield('title', 'Dashboard')</title>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div class="p-5 border-b border-white/10">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 bg-white rounded-xl flex items-center justify-center p-1 flex-shrink-0 shadow-sm overflow-hidden">
                    <img src="{{ asset('assets/logo.png') }}" class="w-full h-full object-contain" alt="Logo">
                </div>
                <div>
                    <p class="text-white font-bold text-sm">{{ config('app.clinic_name', 'Antrix') }}</p>
        ...
        <nav class="flex-1 p-3 space-y-1 overflow-y-auto">
            ...
            <div class="pt-4 border-t border-white/10 mt-4">
                <a href="{{ route('booking') }}" target="_blank" class="sidebar-link">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-4 border-t border-white/10">
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-400 transition-colors">
